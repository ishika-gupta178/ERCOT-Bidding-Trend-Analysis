...
valid_dates = []
two_month_before_dates = []
for date in unique_dates:
    two_month_before_date = date - pd.DateOffset(months=2)
    if two_month_before_date in unique_dates:
        valid_dates.append(date)
        two_month_before_dates.append(two_month_before_date)

dates_df = pd.DataFrame({'Date':valid_dates, 'Two Months Back':two_month_before_dates})
dates_df['Date'] = dates_df['Date'].astype('str')
dates_df['Two Months Back'] = dates_df['Two Months Back'].astype('str')
valid_dates = list(dates_df['Date'].values)
two_month_before_dates = list(dates_df['Two Months Back'].values)
...
def plot_bids(selected_date):
    ...
    two_months_back_date = two_month_before_dates[present_date_index]
    two_months_back_df = df_subset.loc[two_months_back_date]
    ...
    for hour in range(1, 25):
        ...
        two_months_back_hour_data = two_months_back_df[two_months_back_df['Hour Ending'] == hour]
        ...
        back_supply_bids = two_months_back_hour_data[supply_cols].values.flatten()
        back_price_bids = two_months_back_hour_data[price_cols].values.flatten()
    ...
    fig.update_layout(height=800, width=1200, title_text=f'Bids on {selected_date} and 2 Months Back {two_months_back_date}',
                      showlegend=True)
